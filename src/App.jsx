      ...
      prev.map((s) =>
        s.id === slab.id
          ? {
              ...s,
              status: STATUS.CUT,
              history: [...(s.history || []), { event: s.jobName ? `Cut for ${s.jobName}` : "Marked cut & used", date: new Date().toISOString().slice(0, 10) }],
            }
          : s
      )
    );
                ...
                <div className="list-row" key={s.id} onClick={() => setDetailSlab(s)}>
                  ...
                  <div className="list-main">
                    ...
                    <div className="list-sub-line mono">
                      {s.kind === "remnant" ? "Irregular" : `${s.lengthIn}"×${s.widthIn}"`} · {sf ? sf.toFixed(1) : "—"} sqft · {s.thickness} · Lot {s.lotNumber || "—"} · {s.vendor || "—"} · {s.owner}
                    </div>
                  ...
                  <div className="card-body">
                    <div className="card-top">
                      <div>
                        ...
                        <div className="card-sub">{s.thickness} · Lot {s.lotNumber || "—"}</div>
